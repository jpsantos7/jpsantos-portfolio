...
(function (global, factory) {
  ...
  function inViewport(element, axis, value) {
    ...
    let viewportSize, edgeA, edgeB, amount;
  ...
  GreetingClass.prototype.amontList = function () {
    const arr = [];
    const fnc = function (el) {
      const am = el.getAttribute('data-gc-amount');
  ...
  GreetingClass.prototype.dataList = function (dataName) {
    const arr = [];
    const fnc = function (el) {
      const val = el.getAttribute(dataName);
      arr.push(val);
    };
  ...
  GreetingClass.prototype.stateList = function () {
    const fnc = function (el, i) {
      return inViewport(el, this.options.axis, this.amounts[i]);
    }.bind(this);

    const arr = this.elements.map(fnc);
  ...
  GreetingClass.prototype.addClassOnce = function () {
    const arr = [];
    const fnc = function (el) {
      const val = el.getAttribute('data-gc-freq') || this.options.freq;
      arr.push(val);
    }.bind(this);
  ...
  GreetingClass.prototype.mainHandle = function () {
    ...
    if (this.checkList.indexOf(false) === -1) events.off(window, 'scroll resize orientationchange', this.handle);
  };
