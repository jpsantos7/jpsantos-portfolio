...
export default class SiteHeader {
  ...
  scrollToSection(newHash, pushState) {
    ...
    if (sectionEl) {
      ...
      if (isSiteNavigationMobileActive()) {
        anime
  ...
  onKeydownDocument(event) {
    if (event.keyCode === KEYCODES.ESC && isSiteNavigationMobileActive()) {
      event.preventDefault();
      this.closeMobileNavbar();
    }
  }
  ...
  onKeyupDocument(event) {
    if (event.keyCode === KEYCODES.TAB && !isSiteNavigationMobileActive()) {
