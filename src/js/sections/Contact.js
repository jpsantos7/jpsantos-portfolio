import { getEl, queryAll, appendEl, insertAfter } from '../helpers';
import 'whatwg-fetch';
...
export default class Contact {
  ...
  onSubmitContactForm(event) {
    ...
    if (this.isContactFormValid()) {
      ['form__message--visible', 'form__message--error', 'form__message--success'].forEach(className =>
        getEl('#form-live-region').classList.remove(className)
      );
  ...
  clearAllFormErros() {
    ...
    ['form__message--visible', 'form__message--error', 'form__message--success'].forEach(className =>
      liveRegion.classList.remove(className)
    );
  ...
  sendContactEmail() {
    ...
    const nameValue = getEl('#form-name').value;
    const emailValue = getEl('#form-email').value;
    const messageValue = getEl('#form-message').value;

    // eslint-disable-next-line compat/compat
    window
      .fetch('https://formsubmit.co/ajax/d4e85ad2272d8af6ef24892c96c3808c', {
        method: 'POST',
        headers: {
      ...
      .then(response => {
        ...
        if (response.status === 200) {
          ...
          this.showContactFormNotification('error', 'There was a problem sending your message :(');
          this.DOM.contactFormFieldEls.forEach(field => (field.value = ''));
          this.DOM.contactFormSubmitButton.classList.remove('button--spinner');
        }
      })
